...
public class fast_fourier_transformer implements Serializable {
	...
	public Complex[] transformComplex(final short[] f, final TransformType type) {
		final int[][] dataRI = new int[2][f.length];
	...
	public int[] transformInt(final short[] f, final TransformType type) {
		final int[][] dataRI = new int[2][f.length];
		for (int count = 0; count < f.length; count++) {
			dataRI[0][count] = f[count];
		}
        transformInPlace(dataRI, normalization, type);

        return transform_utils.createIntArray(dataRI);
    }

	
	public static void transformInPlace(final int[][] dataRI,
	...
	private static void normalizeTransformedData(final int[][] dataRI,
	        final DftNormalization normalization, final TransformType type) {
